...
import com.ruturaj.springmvc.dao.ContactDAO;
...
public class ContactService {
	...
	public List<ContactDTO> findContactByFirstName(String firstName) {
		 List<ContactDTO> contacts = contactDAO.findContactByFirstName(firstName);
		 if(contacts.size()>0) 
			 return contacts;
		 else
			 return null;
	...
	public List<UserDTO> searchContact(String data, int id) {
		List<UserDTO> contacts = contactDAO.searchContact(data,id);
		return contacts;
	}
