...
import com.ruturaj.springmvc.service.ContactService;
...
public class ContactController {
	...
	@RequestMapping(path = "/search-contact", method = RequestMethod.POST)
	protected String searchContact(@RequestParam(name = "search")String data,ModelMap modelMap, HttpSession httpSession) {
		UserDTO user = (UserDTO) httpSession.getAttribute("user");
		if(user!=null) {
			List<UserDTO> contacts = contactService.searchContact(data,user.getId());
			if(contacts.size()>0) {
				modelMap.addAttribute("contacts", contacts);
				return "contacts";
			}else {			
				modelMap.addAttribute("message", "No contacts found");
				return "contacts";
			}
		}else
			return "login";
